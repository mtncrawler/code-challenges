...
const grandTotal = (stores) => {
  // Solution code here...
  let hourlySales = new Array(12).fill(0);
  
  for(let store of stores){
    for(let i in store){
      hourlySales[i] += store[i];
    }
  }
  return hourlySales;
};
...
const salesData = (data) => {
  ...
  data.forEach(function(elm, ndx) {
    answer.push({ sales: `${elm} cookies`,
                  time: `${hoursOpen[ndx]}`
                })
  })
  return answer;
...
describe('Testing challenge 2', () => {
  test('It should create an object of data for each store', () => {
    expect(salesData(grandTotal(cookieStores), hoursOpen)).toStrictEqual([
